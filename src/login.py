...
def book(login_res, place, datetime, ydtime, num):
    ...
    result_dict = {num: (place_code, times)
                   for num, place_code, times in result}
    ...
    if int(ydtime) in time_code:
        _book(login_res, place_code, datetime.isoformat(),
              "0," + time_map[ydtime], place)
    else:
        print u"该时间段没有空闲场次"
...
def _book(login_res, floor_id, datetime, ydtime, place):
    ...
    r = requests.post(update_url, data=update_post_param,
...
def main():
    ...
    if setting.action == "query":
        results = query_date_range(
            login_res, setting.date_range, setting.place)
        show(results, setting.place)
        return login_res, results
    else:
        book(login_res, place=setting.place, datetime=setting.date,
             ydtime=setting.ydtime, num=setting.floorid)
